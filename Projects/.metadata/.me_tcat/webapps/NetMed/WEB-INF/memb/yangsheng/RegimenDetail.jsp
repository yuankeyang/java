  ...
  $.ajax({
	...
	function succFunction(data) {
	    ...
	    if(data.type=="healFood"){
		     ...
		     $('#content5').html(data.enterprise+s);
	     }
	    if(data.type=="regiComp"){
		     ...
		     $('#content3').html(Yfunction);
		     $('#name4').html("[制作方法]");
		     $('#content4').html(data.makeMethod);
		    // $('#name4').html("[注意事项]");
		    // $('#content4').html(data.precaution);
		     
		     $('#name5').html("[服务机构]");
		     var s="&nbsp;&nbsp;&nbsp;&nbsp;"+"<a href=\"javascript:void(0)\" onclick=\"alert('功能正在实现')\">点击查看详情</a>";
		     $('#content5').html(data.institution+s);
		     ...
		     $('#name2').html("[功效]");
		     $('#content2').html(data.Yfunction);
		     
		     $('#name3').html("[注意事项]");
		     $('#content3').html(data.precaution);
		     ...
		     $('#content1').html(name);
		     $('#name2').html("[适用人群]");
		     $('#content2').html(data.apply);
		     $('#name3').html("[产品型号]");
		     $('#content3').html(data.typeSpec);
