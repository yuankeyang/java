...
var remote = {
	...
	jsjac: {
		httpbase: window.contextPath + "/JHB.servlet", //请求后台http-bind服务器url
		domain: window["serverDomin"], //"192.168.5.231", // 192.168.5.231 当前有效域名
		username: "",
		pass: "",
		timerval: 10000, // 设置请求超时
	...
	login: function (user) {
		...
		try {
			...
			if (user) {
				...
				//alert(user);
				//alert( user.password);
				connectionConfig.username = user.userName;
				connectionConfig.pass = user.password;
				connectionConfig.register = false;//loginForm.register.checked;
			}
			// 连接服务器
